...
Store.prototype.findById = function(id) {
  ...
  return this.model.findOne({id: id}).select("-_id").exec().then(convertToJSON);
};

Store.prototype.read = function(_id) {
  return this.model.findOne({id: _id}).select("-_id").exec().then(convertToJSON);
};
...
Store.prototype.list = function(filter) {
  ...
  return this.model.find(_filter).select("-_id").exec().then(function(arrayOfDocuments) {
